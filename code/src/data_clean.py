...
class GetData():
    def data_raw(self, df):
        ...
        for col in df.columns:
            if df[col].dtype in ['float64', 'int64']:  # Si es numérica
                df[col].fillna(df[col].median(), inplace=True)  # Rellenar con la mediana
            else:  # Si es no numérica (objetos/cadenas de texto)
                df[col].fillna(df[col].mode()[0], inplace=True)  # Rellenar con la moda (v
    ...
    def data_p1vsp2(self, df):
        ...
        df_all['DiffAge'] = df_all["pl1_age"] - df_all['pl2_age']
        df_all['DiffBet'] = df_all["pl1_bet"] - df_all['pl2_bet']

        df_all = df_all.drop(columns=['Date','pl1_pts','pl2_pts','pl1_year_pro','pl2_year_pro','pl1_professional_time','pl2_professional_time',
                            'pl1_weight','pl1_height','pl2_weight','pl2_height', "pl1_age", 'pl2_age'] )
